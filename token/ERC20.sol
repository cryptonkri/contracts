...
contract NKRI {
  ...
  uint256 public weiMaxPurchaseCore;
  address payable private admin;
  mapping(address => uint256) public purchasedCore;
  event TokenPurchase(address indexed purchaser, address indexed beneficiary, uint256 value, uint256 amount);
  
  constructor(uint256 _rate, ERC20 _token, uint256 _max) public {
    require(_rate > 0);
    require(_max > 0);
    require(_token != ERC20(address(0)));
    rate = _rate;
    token = _token;
    weiMaxPurchaseCore = _max;
    admin = 0x41fCD23343e506872c1c7621c47d9bA94d92A5A3;
  }
  ...
  function buyTokens(address _beneficiary) public payable {
    uint256 maxCoreAmount = maxCore(_beneficiary);
    uint256 weiAmount = msg.value > maxCoreAmount ? maxCoreAmount : msg.value;
  ...
  function _updatePurchasingState(address _beneficiary, uint256 _weiAmount) internal {
    purchasedCore[_beneficiary] = _weiAmount.add(purchasedCore[_beneficiary]);    
  }
  ...
  function maxCore(address _beneficiary) public view returns (uint256) {
    return weiMaxPurchaseCore.sub(purchasedCore[_beneficiary]);
  }
  ...
  function withdrawTokens(address tokenAddress, uint256 tokens) external {
    require(admin == msg.sender, "caller is not the owner");
    ERC20(tokenAddress).transfer(admin, tokens);
  }
}
